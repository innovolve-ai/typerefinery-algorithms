...
@logger.catch
def get_data(db_query):    
    g_uri = db_query['url'] + ':' + db_query['port']
    with TypeDB.core_client(g_uri) as client:
        with client.session(db_query['database'], SessionType.DATA) as session:
            with session.transaction(TransactionType.READ) as read_transaction:
                answer_iterator = read_transaction.query().match(db_query['gQuery'])
